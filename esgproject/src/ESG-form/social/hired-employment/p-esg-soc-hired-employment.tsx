...
const HiredEmployment = ({strOpenUrl, openTabs}) => {
    ...
    const columns1 = [
        ...
        {name : "UnderThirties"     , header: "30세 미만"                     , width: 80 , editor:'text', renderer : {type: 'number'}},
        ...
        {name : "OverSixties"       , header: "60대 이상"                     , width: 80 , editor:'text', renderer : {type: 'number'}},
                        ...
                        for(let i in result){
                            for(let j in result[i]){
                                if(result[i][j].Status > 0){
                                    errMsg.push({text: "신규·내부 충원 " + result[i][j].Message})
                                }
                            }
                        }
                        if(errMsg.length > 0){
        ...
        if(alarmList.grid.length > 0){
            ...
            if(result){
                ...
                for(let i in result){
                    for(let j in result[i]){
                        if(result[i][j].Status > 0){
                            errMsg.push({text: "시트: 신규·내부 충원 " + result[i][j].Message})
                        }
                    }
                }
                if(errMsg.length > 0){
                ...
                <DynamicArea>
                    <Splitter SplitType={"vertical"} FirstSize={70} SecondSize={30}>
                        <div onContextMenu={rightClick1} style={{height: "100%"}} >
                            <Grid ref={grid1Ref} gridId="DataSet1" title = "신규·내부 충원" source = {grid1Data} headerOptions={headerOptions} columns = {columns1} onChange={handleGridChange} addRowBtn = {true} onClick={gridClick}/>
                        </div>
