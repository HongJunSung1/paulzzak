...
// 우클릭 조회 시 받는 내부코드 값
let VolunteerCD = 0
let VolunteerTitle = ""; // 파일첨부 제목

const VolunteerEmployment = ({strOpenUrl, openTabs}) => {
    // 로딩뷰
    ...
    // 조회조건 값
    const [VolunteerYear , setCondition1] = useState('');
    ...
    const rightClick1 = (event: React.MouseEvent) => {
        ...
        setTimeout(async () => {
            ...
            if(grid1Ref.current.rightClick() !== null){
                VolunteerCD = grid1Ref.current.rightClick().VolunteerCD;
                VolunteerTitle = grid1Ref.current.rightClick().Year;
            }
            // 조회 조건 담기
            const conditionAr : any[] = [{VolunteerCD : VolunteerCD, DataSet : "FileSet1"}]
            ...
            if(VolunteerCD > 0){
                ...
                try {
                    // 조회 SP 호출 후 결과 값 담기
                    const result = await SP_Request("S_ESG_Soc_Volunteer_File_Query", conditionAr);
    ...
    const deleteFile = async (fileCD) => {
        ...
        setTimeout(async () => {
            try{
                const result = await SP_Request("S_ESG_Soc_Volunteer_File_Cut", [{FileCD: fileCD, VolunteerCD : VolunteerCD, DataSet: "FileSet1"}]);
                if(result){
                    let errMsg : any[] = [];
    ...
    const toolbar = [  
      ...
      , {id: 1, title:"조회", image:"query", spName:"S_ESG_Soc_Volunteer_Query"}
      , {id: 2, title:"저장", image:"save" , spName:"S_ESG_Soc_Volunteer_Save"}
      , {id: 3, title:"삭제", image:"cut"  , spName:"S_ESG_Soc_Volunteer_Cut"}
     ]
    ...
    const columns1 = [
        {name : "VolunteerCD"       , header: "봉사활동코드"        , width: 80 , hidden : true},
            ...
            case 2 :
                ...
                if(fileSaveResult !== null && fileSaveResult !== undefined){
                    for(let i=0;i<fileSaveResult.length;i++){
                        fileSaveResult[i].VolunteerCD = VolunteerCD;
                    }
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgSocVolunteerEmployment') === undefined) {
            setCondition1('');
            setGrid1Data([]);
            setFileData([]);
            setGrid1Changes({DataSet : '', grid: []})
    ...
    useEffect(() => {
      ...
      return (
        <>
        <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgSocVolunteerEmployment' ? "flex" : "none", flexDirection:"column"}}>
                <Loading loading={loading}/>
                <MessageBox messageOpen = {messageOpen} messageClose = {messageClose} MessageData = {message} Title={title}/>
                <Toolbar items={toolbar} clickID={toolbarEvent}/>
                <FixedArea name={"조회 조건"}>
                    <FixedWrap>
                        <DatePick name={"연도"}   value={VolunteerYear}  onChange={setCondition1} width={200} type={"year"} isGrid={false}/>
                ...
                <DynamicArea>
                    <Splitter SplitType={"vertical"} FirstSize={70} SecondSize={30}>
                        <div onContextMenu={rightClick1} style={{height: "100%"}} >
                            <Grid ref={grid1Ref} gridId="DataSet1" title = "봉사활동" source = {grid1Data} headerOptions={headerOptions} columns = {columns1} onChange={handleGridChange} addRowBtn = {true} onClick={gridClick}/>
                        </div>
                        {strOpenUrl === '/PEsgSocVolunteerEmployment' &&
                        <div style={{width: "100%", height: "100%"}} ref={containerRef} >
                            <div style={{height: containerHeight + "px"}}>
                                <File openUrl={strOpenUrl} ref={fileRef} source={fileData} fileCD = {setFileCD} fileTitle={VolunteerTitle}/>
                            </div>
                        </div>}
                    </Splitter>
                </DynamicArea>
