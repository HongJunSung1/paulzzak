...
// 우클릭 조회 시 받는 내부코드 값
let AgeEmpCD = 0
let AgeEmpTitle = ""; // 파일첨부 제목

const AgeEmployment = ({strOpenUrl, openTabs}) => {
    // 로딩뷰
    ...
    // 조회조건 값
    const [AgeEmpYear , setCondition1] = useState('');
    ...
    const rightClick1 = (event: React.MouseEvent) => {
        ...
        setTimeout(async () => {
            ...
            if(grid1Ref.current.rightClick() !== null){
                AgeEmpCD = grid1Ref.current.rightClick().AgeEmpCD;
                AgeEmpTitle = grid1Ref.current.rightClick().Year;
            }
            // 조회 조건 담기
            const conditionAr : any[] = [{AgeEmpCD : AgeEmpCD, DataSet : "FileSet1"}]
            ...
            if(AgeEmpCD > 0){
                ...
                try {
                    // 조회 SP 호출 후 결과 값 담기
                    const result = await SP_Request("S_ESG_Soc_Age_Emp_File_Query", conditionAr);
    ...
    const deleteFile = async (fileCD) => {
        ...
        setTimeout(async () => {
            try{
                const result = await SP_Request("S_ESG_Soc_Age_Emp_File_Cut", [{FileCD: fileCD, AgeEmpCD : AgeEmpCD, DataSet: "FileSet1"}]);
                if(result){
                    let errMsg : any[] = [];
    ...
    const toolbar = [  
      ...
      , {id: 1, title:"조회", image:"query", spName:"S_ESG_Soc_Age_Emp_Query"}
      , {id: 2, title:"저장", image:"save" , spName:"S_ESG_Soc_Age_Emp_Save"}
      , {id: 3, title:"삭제", image:"cut"  , spName:"S_ESG_Soc_Age_Emp_Cut"}
     ]
    ...
    const columns1 = [
        {name : "AgeEmpCD"          , header: "연령별코드"    , width: 80 , hidden : true},
        ...
        {name : "TotalCnt"          , header: "총 인원"       , width: 80 , renderer : {type: 'sum', options: {sumAr : ["UnderThirty", "Thirties", "Fourties", "Fifties", "OverSixties"]}}},
        {name : "UnderThirty"       , header: "30세 미만"     , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "Thirties"          , header: "30대"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "Fourties"          , header: "40대"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "Fifties"           , header: "50대"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "OverSixties"       , header: "60세 이상"     , width: 80 , editor:'text', renderer : {type: 'number'}},
            ...
            case 2 :
                ...
                if(fileSaveResult !== null && fileSaveResult !== undefined){
                    for(let i=0;i<fileSaveResult.length;i++){
                        fileSaveResult[i].AgeEmpCD = AgeEmpCD;
                    }
    ...
    const sendAlarm = async () =>{
        ...
        alarmList.grid.forEach(item => {
            ...
            item.TableKey1 = item.AgeEmpCD; // 공통 키값1(마스터) 추가 없을 경우 0
            item.TableKey2 = 0;             // 공통 키값2(디테일) 추가 없을 경우 0
            delete item.AgeEmpCD; // 기존 키값 삭제
        });
        ...
        if(alarmList.grid.length > 0){
            ...
            if(result){
                ...
                for(let i in result){
                    for(let j in result[i]){
                        if(result[i][j].Status > 0){
                            errMsg.push({text: "시트: 연령별 " + result[i][j].Message})
                        }
                    }
                }
                if(errMsg.length > 0){
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgSocAgeEmployment') === undefined) {
            setCondition1('');
            setGrid1Data([]);
            setFileData([]);
            setGrid1Changes({DataSet : '', grid: []})
    ...
    useEffect(() => {
      ...
      return (
        <>
        <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgSocAgeEmployment' ? "flex" : "none", flexDirection:"column"}}>
                <Loading loading={loading}/>
                <MessageBox messageOpen = {messageOpen} messageClose = {messageClose} MessageData = {message} Title={title}/>
                <Toolbar items={toolbar} clickID={toolbarEvent}/>
                <FixedArea name={"조회 조건"}>
                    <FixedWrap>
                        <DatePick name={"연도"}   value={AgeEmpYear}  onChange={setCondition1} width={200} type={"year"} isGrid={false}/>
                    ...
                    <Splitter SplitType={"vertical"} FirstSize={70} SecondSize={30}>
                        ...
                        <div style={{width: "100%", height: "100%"}} ref={containerRef} >
                            <div style={{height: containerHeight + "px"}}>
                                <File openUrl={strOpenUrl} ref={fileRef} source={fileData} fileCD = {setFileCD} fileTitle={AgeEmpTitle}/>
                            </div>
                        </div>}
                    </Splitter>
                </DynamicArea>
