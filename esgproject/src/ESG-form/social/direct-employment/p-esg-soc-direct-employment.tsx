...
// 우클릭 조회 시 받는 내부코드 값
let DirectEmpCD = 0
let DirectEmpTitle = ""; // 파일첨부 제목

const DirectEmployment = ({strOpenUrl, openTabs}) => {
    // 로딩뷰
    ...
    // 조회조건 값
    const [DirectEmpYear , setCondition1] = useState('');
    ...
    const rightClick1 = (event: React.MouseEvent) => {
        ...
        setTimeout(async () => {
            ...
            if(grid1Ref.current.rightClick() !== null){
                DirectEmpCD = grid1Ref.current.rightClick().DirectEmpCD;
                DirectEmpTitle = grid1Ref.current.rightClick().Year;
            }
            // 조회 조건 담기
            const conditionAr : any[] = [{DirectEmpCD : DirectEmpCD, DataSet : "FileSet1"}]
            ...
            if(DirectEmpCD > 0){
                ...
                try {
                    // 조회 SP 호출 후 결과 값 담기
                    const result = await SP_Request("S_ESG_Soc_Direct_Emp_File_Query", conditionAr);
    ...
    const deleteFile = async (fileCD) => {
        ...
        setTimeout(async () => {
            try{
                const result = await SP_Request("S_ESG_Soc_Direct_Emp_File_Cut", [{FileCD: fileCD, DirectEmpCD : DirectEmpCD, DataSet: "FileSet1"}]);
                if(result){
                    let errMsg : any[] = [];
    ...
    const toolbar = [  
      ...
      , {id: 1, title:"조회", image:"query", spName:"S_ESG_Soc_Direct_Emp_Query"}
      , {id: 2, title:"저장", image:"save" , spName:"S_ESG_Soc_Direct_Emp_Save"}
      , {id: 3, title:"삭제", image:"cut"  , spName:"S_ESG_Soc_Direct_Emp_Cut"}
     ]
    ...
    const complexColumns =[
        {
            header: '정규직',
            name: 'RegularEmp',
            childNames: ['RegularMale', 'RegularFemale', 'RegularTotal', 'RegularPortion']
        },
        {
            header: '비정규직',
            name: 'NonRegularEmp',
            childNames: ['NonRegularMale', 'NonRegularFemale', 'NonRegularTotal', 'NonRegularPortion']
        }
    ]
    ...
    const columns1 = [
        {name : "DirectEmpCD"       , header: "직접고용코드"  , width: 80 , hidden : true},
        ...
        {name : "TotalCnt"          , header: "총 인원"       , width: 80 , renderer : {type: 'sum', options: {sumAr : ["RegularMale", "RegularFemale", "NonRegularMale", "NonRegularFemale"]}}},
        {name : "RegularMale"       , header: "남성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "RegularFemale"     , header: "여성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "RegularTotal"      , header: "합계"          , width: 80 , renderer : {type: 'sum', options:{sumAr : ["RegularMale", "RegularFemale"]}}},
        {name : "RegularPortion"    , header: "비율"          , width: 80 , renderer : {type: 'percent', options:{sumAr: ["RegularMale", "RegularFemale"], divideAr: ["RegularMale", "RegularFemale", "NonRegularMale", "NonRegularFemale"]}}},
        {name : "NonRegularMale"    , header: "남성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "NonRegularFemale"  , header: "여성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "NonRegularTotal"   , header: "합계"          , width: 80 , renderer : {type: 'sum', options:{sumAr : ["NonRegularMale", "NonRegularFemale"]}}},
        {name : "NonRegularPortion" , header: "비율"          , width: 80 , renderer : {type: 'percent', options:{sumAr: ["NonRegularMale", "NonRegularFemale"], divideAr: ["RegularMale", "RegularFemale", "NonRegularMale", "NonRegularFemale"]}}},
            ...
            case 2 :
                ...
                if(fileSaveResult !== null && fileSaveResult !== undefined){
                    for(let i=0;i<fileSaveResult.length;i++){
                        fileSaveResult[i].DirectEmpCD = DirectEmpCD;
                    }
    ...
    const sendAlarm = async () =>{
        ...
        alarmList.grid.forEach(item => {
            ...
            item.TableKey1 = item.DirectEmpCD; // 공통 키값1(마스터) 추가 없을 경우 0
        ...
        if(alarmList.grid.length > 0){
            ...
            if(result){
                ...
                for(let i in result){
                    for(let j in result[i]){
                        if(result[i][j].Status > 0){
                            errMsg.push({text: "시트: 고용 형태(직접고용) " + result[i][j].Message})
                        }
                    }
                }
                if(errMsg.length > 0){
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgSocDirectEmployment') === undefined) {
            setCondition1('');
            setGrid1Data([]);
            setFileData([]);
            setGrid1Changes({DataSet : '', grid: []})
    ...
    useEffect(() => {
      ...
      return (
        <>
        <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgSocDirectEmployment' ? "flex" : "none", flexDirection:"column"}}>
                <Loading loading={loading}/>
                <MessageBox messageOpen = {messageOpen} messageClose = {messageClose} MessageData = {message} Title={title}/>
                <Toolbar items={toolbar} clickID={toolbarEvent}/>
                <FixedArea name={"조회 조건"}>
                    <FixedWrap>
                        <DatePick name={"연도"}   value={DirectEmpYear}  onChange={setCondition1} width={200} type={"year"} isGrid={false}/>
                ...
                <DynamicArea>
                    <Splitter SplitType={"vertical"} FirstSize={70} SecondSize={30}>
                        ...
                        {strOpenUrl === '/PEsgSocDirectEmployment' &&
                        <div style={{width: "100%", height: "100%"}} ref={containerRef} >
                            <div style={{height: containerHeight + "px"}}>
                                <File openUrl={strOpenUrl} ref={fileRef} source={fileData} fileCD = {setFileCD} fileTitle={DirectEmpTitle}/>
                            </div>
                        </div>}
                    </Splitter>
                </DynamicArea>
