...
const WonderWoman = ({strOpenUrl, openTabs}) => {
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgSocSoHotWonderWoman') === undefined) {
            setCondition1('');
            setGrid1Data([]);
            setFileData([]);
            setGrid1Changes({DataSet : '', grid: []})
    ...
    useEffect(() => {
      ...
      return (
        <>
        <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgSocSoHotWonderWoman' ? "flex" : "none", flexDirection:"column"}}>
