...
// 우클릭 조회 시 받는 내부코드 값
let InDirectEmpCD = 0
let InDirectEmpTitle = ""; // 파일첨부 제목

const InDirectEmployment = ({strOpenUrl, openTabs}) => {
    // 로딩뷰
    ...
    // 조회조건 값
    const [InDirectEmpYear , setCondition1] = useState('');
    ...
    const rightClick1 = (event: React.MouseEvent) => {
        ...
        setTimeout(async () => {
            ...
            if(grid1Ref.current.rightClick() !== null){
                InDirectEmpCD = grid1Ref.current.rightClick().InDirectEmpCD;
                InDirectEmpTitle = grid1Ref.current.rightClick().Year;
            }
            // 조회 조건 담기
            const conditionAr : any[] = [{InDirectEmpCD : InDirectEmpCD, DataSet : "FileSet1"}]
            ...
            if(InDirectEmpCD > 0){
                ...
                try {
                    // 조회 SP 호출 후 결과 값 담기
                    const result = await SP_Request("S_ESG_Soc_InDirect_Emp_File_Query", conditionAr);
    ...
    const deleteFile = async (fileCD) => {
        ...
        setTimeout(async () => {
            try{
                const result = await SP_Request("S_ESG_Soc_InDirect_Emp_File_Cut", [{FileCD: fileCD, InDirectEmpCD : InDirectEmpCD, DataSet: "FileSet1"}]);
                if(result){
                    let errMsg : any[] = [];
    ...
    const toolbar = [  
      ...
      , {id: 1, title:"조회", image:"query", spName:"S_ESG_Soc_InDirect_Emp_Query"}
      , {id: 2, title:"저장", image:"save" , spName:"S_ESG_Soc_InDirect_Emp_Save"}
      , {id: 3, title:"삭제", image:"cut"  , spName:"S_ESG_Soc_InDirect_Emp_Cut"}
     ]
    ...
    const complexColumns =[
        {
            header: '일일근로자',
            name: 'DayWorkEmp',
            childNames: ['DayWorkMale', 'DayWorkFemale', 'DayWorkTotal']
        },
        {
            header: '파견근로자',
            name: 'DispatchedEmp',
            childNames: ['DispatchedMale', 'DispatchedFemale', 'DispatchedTotal']
        },
        {
            header: '용역근로자',
            name: 'ServiceEmp',
            childNames: ['ServiceMale', 'ServiceFemale', 'ServiceTotal']
        }
    ]
    ...
    const columns1 = [
        {name : "InDirectEmpCD"     , header: "간접고용코드"  , width: 80 , hidden : true},
        ...
        {name : "TotalCnt"          , header: "총 인원"       , width: 80 , renderer : {type: 'sum', options: {sumAr : ["DayWorkMale", "DayWorkFemale", "DispatchedMale", "DispatchedFemale", "ServiceMale", "ServiceFemale"]}}},
        {name : "DayWorkMale"       , header: "남성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "DayWorkFemale"     , header: "여성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "DayWorkTotal"      , header: "합계"          , width: 80 , renderer : {type: 'sum', options:{sumAr : ["DayWorkMale", "DayWorkFemale"]}}},
        {name : "DispatchedMale"    , header: "남성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "DispatchedFemale"  , header: "여성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "DispatchedTotal"   , header: "합계"          , width: 80 , renderer : {type: 'sum', options:{sumAr : ["DispatchedMale", "DispatchedFemale"]}}},
        {name : "ServiceMale"       , header: "남성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "ServiceFemale"     , header: "여성"          , width: 80 , editor:'text', renderer : {type: 'number'}},
        {name : "ServiceTotal"      , header: "합계"          , width: 80 , renderer : {type: 'sum', options:{sumAr : ["ServiceMale", "ServiceFemale"]}}},
        ...
        switch (clickID){
            ...
            case 1 : 
                    ...
                    const conditionAr : condition = ({
                        InDirectEmpYear : InDirectEmpYear,
                        DataSet  : 'DataSet1'
                    })
            ...
            case 2 :
                ...
                if(fileSaveResult !== null && fileSaveResult !== undefined){
                    for(let i=0;i<fileSaveResult.length;i++){
                        fileSaveResult[i].InDirectEmpCD = InDirectEmpCD;
                    }
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgSocInDirectEmployment') === undefined) {
            setCondition1('');
            setGrid1Data([]);
            setFileData([]);
            setGrid1Changes({DataSet : '', grid: []})
    ...
    useEffect(() => {
      ...
      return (
        <>
        <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgSocInDirectEmployment' ? "flex" : "none", flexDirection:"column"}}>
                <Loading loading={loading}/>
                <MessageBox messageOpen = {messageOpen} messageClose = {messageClose} MessageData = {message} Title={title}/>
                <Toolbar items={toolbar} clickID={toolbarEvent}/>
                <FixedArea name={"조회 조건"}>
                    <FixedWrap>
                        <DatePick name={"연도"}   value={InDirectEmpYear}  onChange={setCondition1} width={200} type={"year"} isGrid={false}/>
                ...
                <DynamicArea>
                    <Splitter SplitType={"vertical"} FirstSize={70} SecondSize={30}>
                        <div onContextMenu={rightClick1} style={{height: "100%"}} >
                            <Grid ref={grid1Ref} gridId="DataSet1" title = "간접고용" source = {grid1Data} headerOptions={headerOptions} columns = {columns1} onChange={handleGridChange} addRowBtn = {true} onClick={gridClick}/>
                        </div>
                        {strOpenUrl === '/PEsgSocInDirectEmployment' &&
                        <div style={{width: "100%", height: "100%"}} ref={containerRef} >
                            <div style={{height: containerHeight + "px"}}>
                                <File openUrl={strOpenUrl} ref={fileRef} source={fileData} fileCD = {setFileCD} fileTitle={InDirectEmpTitle}/>
                            </div>
                        </div>}
                    </Splitter>
                </DynamicArea>
...
export default InDirectEmployment;