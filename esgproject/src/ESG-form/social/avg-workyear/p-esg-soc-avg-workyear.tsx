...
const WorkYear = ({strOpenUrl, openTabs}) => {
    // 로딩뷰
    ...
    const columns1 = [
        {name : "DonationCD"        , header: "평균 근속연수코드"  , width: 80 , hidden : true},
        ...
        {name : "Donation"          , header: "평균 근속연수 (년)", width: 80 , editor:'text', renderer : {type: 'number'}},
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgSocAvgWorkYear') === undefined) {
            setCondition1('');
            setGrid1Data([]);
            setFileData([]);
            setGrid1Changes({DataSet : '', grid: []})
    ...
    useEffect(() => {
      ...
      return (
        <>
        <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgSocAvgWorkYear' ? "flex" : "none", flexDirection:"column"}}>
