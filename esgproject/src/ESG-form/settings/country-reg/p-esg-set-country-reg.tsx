...
const CountryReg = ({strOpenUrl, openTabs}) => {
    // 로딩뷰
    ...
    // 조회조건 값
    const [CountryName, setCondition1] = useState('');
    ...
    const toolbar = [  
      ...
      , {id: 1, title:"조회", image:"query", spName:"S_ESG_Set_Country_Reg_Query"}
      , {id: 2, title:"저장", image:"save" , spName:"S_ESG_Set_Country_Reg_Save"}
      , {id: 3, title:"삭제", image:"cut"  , spName:"S_ESG_Set_Country_Reg_Cut"}
     ]
    ...
    const columns1 = [
        {name : "CountryCD"       , header: "국가코드"    , width: 80  , hidden : false},
        {name : "CountryName"     , header: "국가명"      , width: 200 , editor:'text'},
    ]
                        ...
                        for(let i in result){
                            for(let j in result[i]){
                                if(result[i][j].Status > 0){
                                    errMsg.push({text: "국가 등록 " + result[i][j].Message})
                                }
                            }
                        }
                        if(errMsg.length > 0){
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgSetCountryReg') === undefined) {
            setCondition1('');
            setGrid1Data([]);
            setGrid1Changes({DataSet : '', grid: []})
        }
    ...
    return(
        <>
            <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgSetCountryReg' ? "flex" : "none", flexDirection:"column"}}>
                <Loading loading={loading}/>
                <MessageBox messageOpen = {messageOpen} messageClose = {messageClose} MessageData = {message} Title={title}/>
                <Toolbar items={toolbar} clickID={toolbarEvent}/>     
                <FixedArea name={"조회 조건"}>
                    <FixedWrap>
                        <TextBox name={"국가명"}   value={CountryName} onChange={setCondition1}/>    
                    </FixedWrap>
                </FixedArea>      
                <DynamicArea>
                    <Grid ref={grid1Ref} gridId="DataSet1" title = "국가 정보" source = {grid1Data} headerOptions={headerOptions} columns = {columns1} onChange={handleGridChange} addRowBtn = {true} onClick={gridClick}/>
                </DynamicArea>      
            </div>
        </>
    )
