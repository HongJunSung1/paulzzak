...
const UserGroupForm = ({strOpenUrl, openTabs}) => {
    ...
    // 조회조건 값
    const [GroupName, setCondition1] = useState('');
    ...
    const toolbar = [  
      ...
      , {id: 1, title:"조회", image:"query", spName:"S_UserGroup_Form_Query"}
      , {id: 2, title:"저장", image:"save" , spName:"S_UserGroup_Form_Save"}
     ]
        ...
        switch (clickID){
            ...
            case 2 :
                ...
                for(let i in grid2Changes.grid){
                    grid2Changes.grid[i].GroupCD = GroupCD;
                }
    ...
    const rightClick1 = (event: React.MouseEvent) => {
        ...
        setTimeout(async ()=> {
            ...
            if(grid1Ref.current.rightClick() !== null){
                GroupCD = grid1Ref.current.rightClick().GroupCD
            ...
            if(GroupCD > 0){
                ...
                try {
                    // 조회 SP 호출 후 결과 값 담기
                    const result = await SP_Request("S_UserGroup_Form_SubQuery", [condition2Ar]);
                    if(result.length > 0){
                        // 결과값이 있을 경우 그리드에 뿌려주기
    ...
    return (
        <>
            <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PPzUserGroupForm' ? "flex" : "none", flexDirection:"column"}}>
                <Loading loading={loading}/>
                <MessageBox messageOpen = {messageOpen} messageClose = {messageClose} MessageData = {message} Title={title}/>
                <Toolbar items={toolbar} clickID={toolbarEvent}/>
                <FixedArea name={"조회 조건"}>
                    <FixedWrap>
                        <TextBox   name={"그룹명"}   value={GroupName}  onChange={setCondition1} width={200} searchCode={2} isGrid={false}/>
                ...
                <DynamicArea>
                    <Splitter SplitType={"horizontal"} FirstSize={40} SecondSize={60}>
                        <div onContextMenu={rightClick1} style={{height:"100%"}}>
                            <Grid ref={grid1Ref} gridId="DataSet1" title = "그룹 정보" source = {grid1Data} headerOptions={headerOptions} columns = {columns1} onChange={handleGridChange} addRowBtn = {false} onClick={gridClick}/>
                        </div>
