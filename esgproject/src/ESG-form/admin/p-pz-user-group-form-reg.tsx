...
const UserGroupReg = ({strOpenUrl, openTabs}) => {
    ...
    const toolbar = [  
      ...
      , {id: 1, title:"조회", image:"query", spName:"S_Admin_UserGroup_Query"}
      , {id: 2, title:"저장", image:"save" , spName:"S_Admin_UserGroup_Save"}
      , {id: 3, title:"삭제", image:"cut"  , spName:"S_Admin_UserGroup_Cut"}
     ]
                        ...
                        for(let i in result){
                            for(let j in result[i]){
                                if(result[i][j].Status > 0){
                                    errMsg.push({text: "시트: 사용자그룹 " + result[i][j].Message})
                                }
                            }
                        }
                        if(errMsg.length > 0){
    ...
    return (
        <>
        <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PPzUserGroupFormReg' ? "flex" : "none", flexDirection:"column"}}>
                <Loading loading={loading}/>
                <MessageBox messageOpen = {messageOpen} messageClose = {messageClose} MessageData = {message} Title={title}/>
                <Toolbar items={toolbar} clickID={toolbarEvent}/>
                <FixedArea name={"조회 조건"}>
                    <FixedWrap>
                        <TextBox name={"그룹명"}  value={GroupName} onChange={setCondition1} width={300}/>    
                    </FixedWrap>
                </FixedArea>  
                <DynamicArea>
                    <Grid ref={grid1Ref} gridId="DataSet1" title = "유저그룹 정보" source = {grid1Data} headerOptions={headerOptions} columns = {columns1} onChange={handleGridChange} addRowBtn = {true} onClick={gridClick}/>
                </DynamicArea>
            </div>
        </>
    )
