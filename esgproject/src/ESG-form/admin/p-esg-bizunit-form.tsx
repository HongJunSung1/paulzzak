...
const BizUnitForm = ({strOpenUrl, openTabs}) => {
    ...
    useEffect(() => {
        if (openTabs.find(item => item.url === '/PEsgBizUnitForm') === undefined) {
            setCondition1('');
            setCondition2('');
            setGrid1Data([]);
            setGrid2Data([]);
    ...
    return (
        <>
            <div style={{top: 0 ,height:"100%", display : strOpenUrl === '/PEsgBizUnitForm' ? "flex" : "none", flexDirection:"column"}}>
